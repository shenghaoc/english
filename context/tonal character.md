...
|---|---|---|---|---|
| Minnan (south Fujian) | South coast | Hokkien | High level | Tone 1 (阴平) |
...
English intonation is discourse-functional: pitch marks focus, completion, continuation, and attitude. Southern Min tone systems are lexically specified and reorganized by sandhi over prosodic domains (Chen, 1987; Zhang, 2007). When Minnan speakers produce English, phrase-final pitch habits from sandhi can transfer — high or level finals are common in Southern Min citation tones and right-dominant sandhi domains.

The repo owner is a Fuqing (Hockchia) speaker with a falling-dominant contour habit (Eastern Min, east Fujian). Singapore English and Singlish intonation often sound strikingly similar to Minnan (high-level dominant) rather than to his own variety. That salience is positional: insiders to the dominant south Fujian substrate may not hear the pattern as marked; an Eastern Min outsider hears it as a distinctive melody that overlaps with but does not match his baseline (Preston, 1999). See [rising pitch](<../pronunciation/rising pitch.md>) and [thick accent](<../thick accent.md>).

## Eastern Min vs Southern Min for this repo

Hockchia Close/Open rime alternation (松紧韵) is driven by tone, not by English-style stress or duration. The Southern Min substrate in Singapore English is driven by a different tone inventory and sandhi grammar. The owner's pitch intuitions and the ambient Singlish pitch norm are therefore misaligned in both directions: his natural tendency is not the local default, and the local default is not RP/GA falling declaratives.
